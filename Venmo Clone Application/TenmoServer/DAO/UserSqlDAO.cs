...
namespace TenmoServer.DAO
{
    public class UserSqlDAO : IUserDAO
    {
        ...
        public List<UserDTO> GetUserDTOs()
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    SqlCommand cmd = new SqlCommand("SELECT username, u.user_id, account_id FROM users u JOIN accounts a ON a.user_id = u.user_id", conn);
                    SqlDataReader reader = cmd.ExecuteReader();
                    ...
                    if (reader.HasRows)
                    {
                        while (reader.Read())
                        {
                            UserDTO u = GetUserDTOFromReader(reader);
                            returnUsers.Add(u);
                        }
                    }
                }
        ...
        private UserDTO GetUserDTOFromReader(SqlDataReader reader)
        {
